...
public class Home extends AppCompatActivity {
    ...
    TextView textView2, ifsc1 , username, mailid,aadhaar;
    Intent intent;
    String path;
    ImageView propic;
    Firebase fb;
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        //textView2 = (TextView) findViewById(R.id.textView2);
        username= (TextView)findViewById(R.id.cus_name);
        mailid=(TextView)findViewById(R.id.cus_email);
        aadhaar=(TextView)findViewById(R.id.cus_aadhar);
        propic=(ImageView)findViewById(R.id.Propic);

        calligrapher = new Calligrapher(this);
        calligrapher.setFont(Home.this,"Ubuntu_R.ttf",true);
...
//        textView2.setTypeface(typeface);
        ...
        username.setText(sharedPreferences.getString("cusname",""));
        mailid.setText(sharedPreferences.getString("cusemail",""));
        aadhaar.setText(sharedPreferences.getString("aadhaar",""));
        StorageReference storagereference= FirebaseStorage.getInstance().getReference().child("Customer").child(sharedPreferences.getString("aadhaar",""));
        storagereference.child("cus.jpg").getDownloadUrl().addOnSuccessListener(new OnSuccessListener<Uri>() {
            @Override
            public void onSuccess(Uri uri) {
                String downurl= uri.toString();
                System.out.println("cusurl is : "+ downurl);
                Glide.with(getApplicationContext()).load(downurl).into(propic);
        ...
        storagereference= FirebaseStorage.getInstance().getReference().child("Jewel");
    ...
    private void registerClickCallback() {
        ...
        list.setOnItemClickListener(new AdapterView.OnItemClickListener() {
            ...
            public void onItemClick(AdapterView<?> parent, View view, int position, long id) {
                ...
                final String item = myJewels.get(position).getIJSC();


                // String img = clickedJewel.getIconID();
                String type =  myJewels.get(position).getJeweltype();
                String price =  myJewels.get(position).getPrice();
                String certified= myJewels.get(position).getCertified();
                String weight= myJewels.get(position).getWeight();
                String size= myJewels.get(position).getSize();
                String color= myJewels.get(position).getColor();
                storagereference= FirebaseStorage.getInstance().getReference().child("Jewels").child(item);



                final Dialog dialog = new Dialog(Home.this);

                System.out.println("Bowwwwwwww"+myJewels.get(position).getIJSC()+myJewels.get(position).getJeweltype()+myJewels.get(position).getPrice()+myJewels.get(position).getCertified()+myJewels.get(position).getWeight()+myJewels.get(position).getSize()+myJewels.get(position).getColor());
                dialog.requestWindowFeature(Window.FEATURE_NO_TITLE);
                dialog.setContentView(R.layout.activity_jewel_details);
                TextView ifsc=(TextView)dialog.findViewById(R.id.ifsc1);
                ...
                storagereference.child("jewel.jpg").getDownloadUrl().addOnSuccessListener(new OnSuccessListener<Uri>() {
                    @Override
                    public void onSuccess(Uri uri) {
                        String downurl= uri.toString();
                        System.out.println("url is : "+ downurl);
                        Glide.with(getApplicationContext()).load(downurl).into(imageView);
...
//        });
    ...
    private class MyListAdapter extends ArrayAdapter<Jewel> {
        ...
        public View getView(int position, View convertView, ViewGroup parent) {
            ...
            StorageReference sr=FirebaseStorage.getInstance().getReference().child("Jewels").child(currentJewel.getIJSC());
            final View finalItemView = itemView;
            sr.child("jewel.jpg").getDownloadUrl().addOnSuccessListener(new OnSuccessListener<Uri>() {
                @Override
                public void onSuccess(Uri uri) {
                    String downurl= uri.toString();
                    System.out.println("downurl  : "+ downurl);
                    ImageView jewelimage = (ImageView) finalItemView.findViewById(R.id.item_icon);
                    Glide.with(getApplicationContext()).load(downurl).into(jewelimage);
                ...
                public void onDataChange(DataSnapshot dataSnapshot) {
                    if(dataSnapshot!=null){
                        ...
                        for(DataSnapshot child:dataSnapshot.getChildren()){
                            ...
                            myJewels.add(new Jewel(adap.getJeweltype(),adap.getIJSC(),adap.getPrice(),adap.getColor(),adap.getSize(),adap.getWeight(),adap.getCertified()));
                        }
